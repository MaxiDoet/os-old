...
void ip_handle_packet(uint8_t *buffer)
{
	...
	header->version = (version_ihl & 0x0F);
	header->ihl = ((version_ihl & 0xF0) >> 4);
...
void ip_send_packet(uint8_t *src_ip, uint8_t *dst_ip, uint8_t *data, uint32_t size) {
	...
	header->ihl = 5;
	header->tos = 0;
	header->length = net_swap_word(sizeof(ip_packet_header) + size);
	header->id = 0;
	header->flags = 0;
	header->ttl = 64;
	header->protocol = 17;
	...
	// Check if this a broadcast
	if (!(dst_ip[0] == 0xFF && dst_ip[1] == 0xFF && dst_ip[2] == 0xFF && dst_ip[3] == 0xFF)) {
